...
import { deferredImageLoadingProps, heroImageLoadingProps } from "@shared/imageLoading";
  ...
  return (
    <main dir="rtl" className="landing-shell">
      ...
      <section id="examples" className="examples-section page-width">
        ...
        <div className="evidence-grid">
          {evidence.map(item => <article className="evidence-card" key={item.title}>
            <div className="evidence-images"><div><span>الصورة الأصلية</span><img src={item.before} alt={`الصورة الأصلية لطفل قصة ${item.title}`} {...deferredImageLoadingProps} /></div><strong>←</strong><div><span>من القصة</span><img src={item.pages[0]} alt={`صفحة من قصة ${item.title}`} {...deferredImageLoadingProps} /></div></div>
        ...
        <div className="illustrative-wrap"><div className="illustrative-line"><span>أمثلة توضيحية للشكل الممكن</span></div><p>الأمثلة التالية توضح تنوع العوالم والأساليب فقط، وليست قصص عملاء حقيقيين.</p><div className="mini-grid">{miniExamples.map(([image, title]) => <figure key={title}><img src={image} alt={`مثال توضيحي لشخصية قصصية: ${title}`} {...deferredImageLoadingProps} /><figcaption><b>{title}</b><span>مثال توضيحي</span></figcaption></figure>)}</div></div>
      </section>
      ...
      <section className="case-study page-width">
        ...
        <div className="case-visual"><div className="case-photo"><span>قبل</span><img src={ASSETS.ahmadOriginal} alt="الصورة الأصلية لأحمد" {...deferredImageLoadingProps} /></div><div className="case-arrow">←</div><div className="case-pages"><span>بعد</span><img src={ASSETS.ahmadPage1} alt="غلاف قصة أحمد" {...deferredImageLoadingProps} /><img src={ASSETS.ahmadPage2} alt="صفحة من قصة أحمد" {...deferredImageLoadingProps} /><img src={ASSETS.ahmadPage3} alt="صفحة ختامية من قصة أحمد" {...deferredImageLoadingProps} /></div></div>
      </section>
