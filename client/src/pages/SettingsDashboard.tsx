...
export default function SettingsDashboard() {
  ...
  const update = trpc.settings.update.useMutation({ onSuccess: data => { setForm(data); setSaved(true); void settings.refetch(); toast.success("تم حفظ الإعدادات."); } });
  ...
  const reset = () => { if (defaults.data) { setForm(defaults.data); setSaved(false); toast.info("تمت تعبئة القيم الافتراضية. اضغطي حفظ الإعدادات لتطبيقها."); } };
  ...
  return <AdminLayout title="الإعدادات" description="غيّري محتوى الموقع وسعره وبيانات التتبع من هنا، بدون الرجوع إلى GitHub أو تعديل ملفات برمجية.">
    {settings.isLoading || !form ? <div className="admin-empty"><Settings2 size={30} className="spin" /><p>جاري تحميل الإعدادات…</p></div> : <form className="settings-form" onSubmit={submit}>
      ...
      <div className="settings-actions"><button type="submit" className="admin-save-button" disabled={update.isPending}><Save size={15} /> {update.isPending ? "جاري الحفظ…" : "حفظ الإعدادات"}</button><button type="button" className="admin-ghost-button" onClick={reset}><RotateCcw size={15} /> تعبئة القيم الافتراضية</button>{saved && <span className="settings-saved"><CheckCircle2 size={15} /> تم الحفظ</span>}</div>
    </form>}
  </AdminLayout>;
}
