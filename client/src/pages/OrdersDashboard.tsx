...
export default function OrdersDashboard() {
  ...
  const sendReply = trpc.telegram.sendReply.useMutation({ onSuccess: () => setReplyMessage("") });

  const filteredOrders = useMemo(() => {
    const allOrders = orders.data ?? [];
    const normalized = query.trim().toLowerCase();
    if (!normalized) return allOrders;
    return allOrders.filter(order => [order.reference, order.childName, order.childInterest, order.contactValue, order.referralCode ?? ""].join(" ").toLowerCase().includes(normalized));
  }, [orders.data, query]);
  ...
  return <AdminLayout title="الطلبات" description="تابعي كل طلب من أول محادثة إلى التسليم، وسجّلي قيمته المالية من نفس المكان.">
    ...
    <section className="admin-stat-card" style={{ marginBottom: "1.5rem", textAlign: "right" }}>
      ...
      <div className="order-form-grid">
        <label>معرّف محادثة العميل<input inputMode="numeric" value={replyChatId} onChange={event => setReplyChatId(event.target.value)} placeholder="يظهر في إشعار الطلب" /></label>
