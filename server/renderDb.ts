...
export type RenderOrderRecord = RenderOrder & {
  status: OrderStatus;
...
export async function updateRenderConversationOrder(reference: string, status: OrderStatus, adminNotes: string, orderAmount: number, paymentStatus: PaymentStatus) {
  await getRenderPool().query(
    "UPDATE conversation_orders SET status = $2, admin_notes = $3, order_amount = $4, payment_status = $5, updated_at = NOW() WHERE reference = $1",
    [reference, status, adminNotes.trim() || null, orderAmount, paymentStatus],
  );
}

export async function createRenderExpense(expense: ExpenseInput) {
  const result = await getRenderPool().query<RenderExpenseRecord>(
    `INSERT INTO finance_expenses (description, category, amount, expense_date, notes)
     VALUES ($1, $2, $3, $4, $5)
     RETURNING id, description, category, amount::float8 AS amount, expense_date::text AS "expenseDate", notes, created_at AS "createdAt", updated_at AS "updatedAt"`,
    [expense.description, expense.category, expense.amount, expense.expenseDate, expense.notes.trim()],
  );
  return result.rows[0];
}

export async function listRenderExpenses(month?: string) {
  const result = await getRenderPool().query<RenderExpenseRecord>(
    `SELECT id, description, category, amount::float8 AS amount, expense_date::text AS "expenseDate", notes, created_at AS "createdAt", updated_at AS "updatedAt"
     FROM finance_expenses
     ${month ? "WHERE TO_CHAR(expense_date, 'YYYY-MM') = $1" : ""}
...
export async function deleteRenderExpense(id: number) {
  await getRenderPool().query("DELETE FROM finance_expenses WHERE id = $1", [id]);
}

export async function getRenderMonthlySummary(month: string) {
  const [orders, expenses, categories] = await Promise.all([
    getRenderPool().query<{ orderCount: string; paidOrderCount: string; orderValue: string; collectedRevenue: string }>(
      `SELECT
        COUNT(*)::int AS "orderCount",
        COUNT(*) FILTER (WHERE COALESCE(payment_status, 'unpaid') = 'paid')::int AS "paidOrderCount",
        COALESCE(SUM(COALESCE(order_amount, 0)), 0)::float8 AS "orderValue",
        COALESCE(SUM(CASE WHEN COALESCE(payment_status, 'unpaid') = 'paid' THEN COALESCE(order_amount, 0) ELSE 0 END), 0)::float8 AS "collectedRevenue"
       FROM conversation_orders
       WHERE TO_CHAR(created_at, 'YYYY-MM') = $1`,
      [month],
    ),
    getRenderPool().query<{ totalExpenses: string }>(
      `SELECT COALESCE(SUM(amount), 0)::float8 AS "totalExpenses"
       FROM finance_expenses
       WHERE TO_CHAR(expense_date, 'YYYY-MM') = $1`,
      [month],
    ),
    getRenderPool().query<{ category: string; amount: string }>(
  ...
  const totalExpenses = Number(expenses.rows[0]?.totalExpenses ?? 0);
  const collectedRevenue = Number(orderSummary.collectedRevenue ?? 0);

  return {
    month,
    orderCount: Number(orderSummary.orderCount ?? 0),
    paidOrderCount: Number(orderSummary.paidOrderCount ?? 0),
    orderValue: Number(orderSummary.orderValue ?? 0),
    collectedRevenue,
    totalExpenses,
    netProfit: collectedRevenue - totalExpenses,
    expenseCategories: categories.rows.map(item => ({ category: item.category, amount: Number(item.amount) })),
  };
}
