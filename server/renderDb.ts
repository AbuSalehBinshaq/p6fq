...
export type RenderMarketerRecord = { id: number; name: string; code: string; commissionPercent: number; active: boolean; createdAt: Date };
...
export async function migrateRenderDatabase() {
  ...
  await getRenderPool().query(`CREATE TABLE IF NOT EXISTS referral_partners (id BIGSERIAL PRIMARY KEY, code VARCHAR(48) NOT NULL UNIQUE, name VARCHAR(120) NOT NULL, commission_type VARCHAR(20) NOT NULL DEFAULT 'fixed', commission_value NUMERIC(12, 2) NOT NULL DEFAULT 0, active BOOLEAN NOT NULL DEFAULT TRUE, created_at TIMESTAMPTZ NOT NULL DEFAULT NOW(), updated_at TIMESTAMPTZ NOT NULL DEFAULT NOW());`);
...
export async function listRenderConversationOrders() {
  const result = await getRenderPool().query<RenderOrderRecord>(`SELECT o.reference, o.child_name AS "childName", o.child_age AS "childAge", o.child_interest AS "childInterest", o.contact_method AS "contactMethod", o.contact_value AS "contactValue", o.privacy_consent AS "privacyConsent", o.referral_code AS "referralCode", p.name AS "partnerName", o.status, o.admin_notes AS "adminNotes", COALESCE(o.order_amount, 0)::float8 AS "orderAmount", COALESCE(o.payment_status, 'unpaid') AS "paymentStatus", o.created_at AS "createdAt", o.owner_notified_at AS "ownerNotifiedAt", o.telegram_opened_at AS "telegramOpenedAt" FROM conversation_orders o LEFT JOIN referral_partners p ON p.code = o.referral_code ORDER BY o.created_at DESC`);
  return result.rows;
}
...
export async function listRenderMarketers() { const result = await getRenderPool().query<RenderMarketerRecord>(`SELECT id, name, code, commission_percent::float8 AS "commissionPercent", active, created_at AS "createdAt" FROM marketers ORDER BY active DESC, created_at DESC`); return result.rows; }
export async function createRenderMarketer(input: { name: string; code: string; commissionPercent: number }) { const result = await getRenderPool().query<RenderMarketerRecord>(`INSERT INTO marketers (name, code, commission_percent) VALUES ($1, $2, $3) RETURNING id, name, code, commission_percent::float8 AS "commissionPercent", active, created_at AS "createdAt"`, [input.name.trim(), input.code.trim().toLowerCase(), input.commissionPercent]); return result.rows[0]; }
export async function updateRenderMarketer(id: number, input: { name: string; code: string; commissionPercent: number; active: boolean }) { const result = await getRenderPool().query<RenderMarketerRecord>(`UPDATE marketers SET name = $2, code = $3, commission_percent = $4, active = $5 WHERE id = $1 RETURNING id, name, code, commission_percent::float8 AS "commissionPercent", active, created_at AS "createdAt"`, [id, input.name.trim(), input.code.trim().toLowerCase(), input.commissionPercent, input.active]); return result.rows[0]; }
