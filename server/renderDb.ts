...
export async function createRenderShortLink(input: { slug: string; partnerCode: string; source: string; campaign: string; content: string }) { const result = await getRenderPool().query<RenderShortLinkRecord>(`INSERT INTO short_links (slug, partner_code, source, campaign, content) VALUES ($1, $2, $3, $4, $5) RETURNING id, slug, partner_code AS "partnerCode", source, campaign, content, clicks, active, created_at AS "createdAt"`, [input.slug, input.partnerCode, input.source, input.campaign, input.content]); return result.rows[0]; }
export async function listRenderShortLinks() { const result = await getRenderPool().query<RenderShortLinkRecord>(`SELECT id, slug, partner_code AS "partnerCode", source, campaign, content, clicks, active, created_at AS "createdAt" FROM short_links ORDER BY created_at DESC`); return result.rows; }
export async function getRenderShortLink(slug: string) { const result = await getRenderPool().query<RenderShortLinkRecord>(`UPDATE short_links SET clicks = clicks + 1 WHERE slug = $1 AND active = TRUE RETURNING id, slug, partner_code AS "partnerCode", source, campaign, content, clicks, active, created_at AS "createdAt"`, [slug]); return result.rows[0] ?? null; }
